...
<div class="page-content fade-in-up">
  <form method="post" action="{{route('article.update', $detail->id)}}" enctype="multipart/form-data">
    @csrf
    @method('put')
                                  ...
                                  <div class="form-group col-md-4">
                                        ...
                                        <select name="articletype_id" class="form-control">
                                          ...
                                          @foreach($articleType as $category)
                                            <option value="{{$category->id}}" {{$detail->articletype_id == $category->id ? 'selected' : ''}}>{{$category->title}}</option>
                                          @endforeach
                                        </select>
                                  </div>
                              </div>
