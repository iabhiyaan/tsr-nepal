...
<div class="page-content fade-in-up">
    <div class="ibox">
        <div class="ibox-head">
            <div class="ibox-title">All Podcast</div>
            <div>
                <a class="btn btn-info btn-md" href="{{route('podcast.create')}}">Add Podcast</a>
            </div>
        </div>
                ...
                <tbody>
                  ...
                  @if($details->count())
                    ...
                    <tr>
                      ...
                      <td><iframe width="200" height="100" src="{{$data->link}}" frameborder="3" gesture="media" allow="encrypted-media" allowfullscreen></iframe></td>

                      <td>{{$data->published == 1 ? 'Published' : 'Not Published'}}</td>
                      <!-- <td>{{$data->homepage == 1 ? 'Yes' : 'No'}}</td> -->

                        <td>
                            <a href="{{route('podcast.edit', $data->id)}}" class="btn btn-success btn-sm"><i class="fa fa-edit"></i></a>

                              <form class="adjust-delete-button" action="{{route('podcast.destroy', $data->id)}}" method="post">
                                  @csrf
                                  @method('delete')
